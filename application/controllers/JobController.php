...
class JobController extends MY_Controller {
    ...
    public function getQxc($expect=''){
        if($expect){
            ...
            if(!$row){
                ...
                $this->keepQxc(str_pad($row['expect']+1, 5, 0, STR_PAD_LEFT));
            }
        }
    }
    ...
    public function keepQxc($expect){
        ...
        if($this->db->where('expect', $lottery['expect'])->get('app_qxc')->num_rows()>0){
            echo $expect . ' exists' . "\n";
            return false;
        }
        
        if($this->db->insert('app_qxc', $lottery)){
            echo $expect . ' keep ok'. "\n";
        }
    }
    ...
    public function getSsq($expect=''){
        if($expect){
            ...
            if(!$row){
                ...
                $this->keepSsq(str_pad($row['expect']+1, 5, 0, STR_PAD_LEFT));
            }
        }
    }
