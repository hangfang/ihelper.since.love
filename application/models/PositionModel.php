...
EOF;
    ...
    public function searchAround($lastMsg, $msgXml){
        ...
        $data = array();
        $data['method'] = 'get';
        $data['url'] = sprintf(TENCENT_MAP_APP_URL.'/place/v1/search?boundary=nearby(%s,%s,1000)&keyword=%s&page_size=6&page_index=1&orderby=_distance&key=%s', $lastMsg['Location_X'], $lastMsg['Location_Y'], $msgXml['Content'], TENCENT_MAP_APP_KEY);
